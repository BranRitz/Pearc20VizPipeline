...
def pullpublicschool(covid_data):
    '''
    ...
    for state in states:
        ...
        for school in schoollist:
            ...
            print(attr["CNTY"])
            school_name_arr.append(attr["NAME"]) + covid_data.query('fips=="'+str(attr["CNTY"])+'"')["cases"]
            ...
            case_arr.append(covid_data['fips']['cases'])
    ...
    diction = {"name": school_name_arr, "street": street_addr_arr, "city": city_arr,
               ...
               "lon": lon_arr, "cases": case_arr}
    return json.dumps(diction)


def generateSchoolMap(covid_today):
    # load data first time only
    if (not path.exists("schools.json")):
        schoollist = pullpublicschool(covid_today)
